...
export async function appendToSheet(order) {
  // Usa o endpoint /api/sheets.js existente (já configurado com Apps Script)
  ...
  const payload = {
    ...
    customer_phone: order.customer_phone,
    qty_40: order.qty_40,
    qty_240: order.qty_240,
    qty_500: order.qty_500,
    subtotal: order.subtotal,
    frete: order.frete,
    desconto: order.desconto,
    total: order.total_amount,
    cupom: order.cupom_code || '',
    payment_method: order.payment_method,
    address: [addr.street, addr.number, addr.complement, addr.neighborhood, addr.city, addr.state, addr.cep]
      .filter(Boolean).join(', '),
    date: new Date(order.paid_at || Date.now()).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
    order_id: order.pedido_id || order.id.slice(0, 8),
  };
